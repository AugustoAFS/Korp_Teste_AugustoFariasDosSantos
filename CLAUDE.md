...
| Path | Role | Stack / DB |
|---|---|---|
| `API-Gateway/` (`Gateway`) | Single entry point: auth (Argon2id + HttpOnly cookie), YARP reverse proxy, signs internal JWT for downstream, Polly circuit breaker | .NET 10, MVC (no Clean Arch — no domain of its own), PostgreSQL |
...
- **MassTransit conventions** (`EventListenerService.AddEventListeners`). `AddConsumers(assembly)` scans for consumers; `ConfigureEndpoints` creates queues from the nested `ConsumerDefinition` in each `On*` listener — **adding a listener needs no change to the registration code**. Each message contract carries `[MessageUrn("urn:message:emissor:...")]`; the URN **must be identical on both the publishing and consuming service**, or the message sits in the queue with no consumer.
...
- **Message URNs are a contract.** When adding or changing a cross-service message, update the `[MessageUrn]` on both sides together, and add its CLR type to the dispatcher's type map in `OutboxDispatcherWorker`.
- Committed `appsettings.json` files are still defaults — connection strings (`ConnectionStrings:RabbitMq`, DBs) are expected via `appsettings.Development.json` / user secrets, not committed.
