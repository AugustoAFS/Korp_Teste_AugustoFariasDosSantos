...
| Path | Role | Stack / DB |
|---|---|---|
| `API-Gateway/` (`Gateway`) | Single entry point: auth (Argon2id + HttpOnly cookie), YARP reverse proxy, signs internal JWT for downstream, Polly circuit breaker per cluster | .NET 10, MVC (no Clean Arch — no domain of its own), PostgreSQL |
...
- **A stalled print keeps its `processing_id`.** `PrintExpirationWorker` only writes `last_error` on an invoice that has been printing for over 60s — it never clears `processing_id`. That single decision is what makes the saga self-healing: a late `EstoqueBaixado`/`EstoqueRejeitado` still matches the invoice and applies, and a user-triggered retry (`RestartPrinting`) republishes under the **same** key. Never "clean up" that field on expiry.

- **A duplicate `BaixarEstoqueCommand` replays the stored outcome — it is not a no-op.** `processed_messages` in Estoque carries `outcome_type` + `outcome_payload`, written in the *same transaction* as the debit. When `OnBaixarEstoque` sees a key it already processed, `ReplayOutcome` re-enqueues that exact event into the outbox instead of swallowing it. Without this the saga had a permanent hole: if a result event were ever lost for good, the retry would be deduped, nothing would be republished, and the invoice would stay stuck forever. Two call sites look alike but are not: `ReplayOutcome` (the marker already existed) republishes; `SkipConcurrent` (lost the insert race) only rolls back, because the consumer that won will publish its own result — and because the failed `ProcessedMessage` is still Added in the change tracker, saving there would fail again.
