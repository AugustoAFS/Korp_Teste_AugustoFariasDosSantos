...
| `codigo` | `string` | — |
| `descricao` | `string` | — |
| `saldo` | `number` | inteiro `>= 0` |
| `ativo` | `boolean` | — |

`ProdutoRequest` leva só `codigo`, `descricao` e `saldo` — os três obrigatórios.
...
Parâmetros de consulta: `pagina` (padrão 1), `tamanho` (padrão 20, teto 100), `busca`.
...
| `GET` | `/api/v1/produtos?pagina&tamanho&busca` | `PaginaDe<Produto>` |
...
| `buscar(termo)` | `Produto[]` — typeahead, mesmo endpoint com `tamanho: 10` |
