...
namespace Estoque.TestesUnitarios.Controllers;
...
public sealed class BaseControllerTests
{
    ...
    public BaseControllerTests()
    {
        ...
        contexto.Request.Path = "/api/v1/produtos";
    ...
    {
        var resposta = _controller.Executar(Result<string>.Ok("PAR-M8"));

        var objeto = resposta.ShouldBeOfType<ObjectResult>();
        objeto.StatusCode.ShouldBe((int)HttpStatusCode.OK);
        objeto.Value.ShouldBe("PAR-M8");
    }

    [Fact]
    public void Created_preserva_o_201()
    {
        var resposta = _controller.Executar(Result<string>.Created("PAR-M8"));
    ...
    {
        var resposta = _controller.Executar((Result)Errors.ProductNotFound);
    ...
    public void Erro_expoe_o_code_legivel_por_maquina()
    {
        ...
        problema.Extensions["code"].ShouldBe("product_not_found");
    }
    ...
    public void Erro_registra_o_caminho_requisitado_em_instance()
    {
        ...
        var problema = resposta.ShouldBeOfType<ObjectResult>().Value.ShouldBeOfType<ProblemDetails>();
        problema.Instance.ShouldBe("/api/v1/produtos");
    }

    [Fact]
    public void Erro_leva_titulo_e_detalhe_do_catalogo_para_o_usuario()
    {
        var resposta = _controller.Executar((Result)Errors.ProductWithBalance);

        var problema = resposta.ShouldBeOfType<ObjectResult>().Value.ShouldBeOfType<ProblemDetails>();
        problema.Title.ShouldBe(Errors.ProductWithBalance.Title);
        problema.Detail.ShouldBe(Errors.ProductWithBalance.Detail);
    }

    [Fact]
    public void Detalhe_enriquecido_com_With_chega_na_resposta()
    {
        var erro = Errors.InsufficientBalance.With("Faltam 2 unidades de PAR-M8.");

        var resposta = _controller.Executar((Result)erro);

        var problema = resposta.ShouldBeOfType<ObjectResult>().Value.ShouldBeOfType<ProblemDetails>();
        problema.Detail.ShouldBe("Faltam 2 unidades de PAR-M8.");
    }

    [Fact]
    public void Result_generico_com_erro_nao_devolve_valor()
    {
        var resposta = _controller.Executar((Result<string>)Errors.ProductNotFound);
