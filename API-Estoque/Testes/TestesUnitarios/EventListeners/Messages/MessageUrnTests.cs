...
namespace Estoque.TestesUnitarios.EventListeners.Messages;
...
public sealed class MessageUrnTests
{
    ...
    public void Urn_de_cada_contrato_e_exatamente_a_combinada_com_o_faturamento(Type contrato, string esperada)
        => MessageUrn.ForTypeString(contrato).ShouldBe(esperada);

    [Fact]
    public void Nenhuma_urn_se_repete_entre_contratos()
    {
        var urns = Contratos.Select(MessageUrn.ForTypeString).ToArray();

        urns.Distinct().Count().ShouldBe(Contratos.Length);
    }
    ...
    public void Contrato_e_serializavel_sem_disparar_a_validacao_do_atributo()
    {
        ...
        Should.NotThrow(() => System.Text.Json.JsonSerializer.Serialize(evento));
    }
}
